...
### Fixed
- **Radarr / Sonarr library pages report the instance unreachable even though the connection test passes** ([#41](https://github.com/Shoshuo/Prismarr/issues/41)). Two-part fix. (1) *The breaker no longer confuses "slow" with "down":* the heavy `getMovies()` / `getSeries()` payload is fetched through the shared `get()`, and a read timeout there used to execute the same mark-down branch as a connection failure — so a big library on slow storage tripped the per-instance circuit breaker and every subsequent call short-circuited to "unreachable", while the lightweight connection *test* hit a fast endpoint and passed. `get()` and `multiGet()` now distinguish the two: a timeout **after a successful connect** fails only that call (and still short-circuits the rest of the same request to avoid stacking waits) but never opens the cross-request breaker; connection-level failures (refused / DNS / connect timeout) trip it exactly as before. (2) *The library routes get room to finish:* the full-library fetches accept a per-call timeout, and the library/collections/filter/search-index routes — which already allow `set_time_limit(120)` — pass a generous 30 s (`LIBRARY_TIMEOUT`). Frequent paths (dashboard widgets, quick-look membership checks, admin statistics) keep the old 8 s / 4 s bound so one slow instance can't pin their requests for 30 s.
