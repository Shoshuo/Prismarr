...
### Fixed
- **Transmission detail panel always showed 0 B/s.** `getTorrentDetail()` read `rateDownload`/`rateUpload` off the torrent object for the General tab's speeds, but the `torrent-get` RPC's field list (`DETAIL_FIELDS`) never requested them — and Transmission only returns fields that were explicitly asked for, so both coalesced to `0` regardless of actual transfer speed. The two fields are now included in `DETAIL_FIELDS`; a reflection-based test asserts the list contains every field the detail mapper reads.
