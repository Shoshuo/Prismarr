...
### Fixed
- **A torrent page kept polling — and overwriting — after you navigated to a different torrent client.** Each of the qBittorrent, Deluge and Transmission pages refreshes on a 3-second `setInterval`, but the timer handle lived in the page script's own closure. Turbo Drive re-executes the incoming page's `<script>` with a fresh closure while the outgoing page's closure — and its live interval — survive, so `clearInterval` could only ever reach the timer of the script instance calling it, never the orphan left behind. Because all three pages render the *same* element ids (`qbt-list`, `qbt-stat-total`, …), the stale poller's existence checks still passed against its successor's DOM: after switching from Deluge to Transmission, Deluge's orphaned poller kept fetching its own torrents every 3 s and painting them over the Transmission page, the two pollers overwriting each other at 3 s apiece — which reads as data corruption ("Transmission is showing my Deluge torrents"). The handle now lives on a single shared global (`window._prismarrTorrentPagePollTimer` — one name serves all three, since only one torrent page is ever mounted), so the incoming page's `startRefresh()` reaches and kills the outgoing page's timer, and it is registered in the `turbo:before-render` cleanup so leaving for a **non**-torrent page — which re-executes no torrent script and would otherwise never clear it — stops the interval too. Verified live: Deluge (54 torrents) → Transmission (0) now yields 18 Transmission polls and zero Deluge polls over 51 s with the count holding at 0, and the reverse direction holds 54; a torrent page → dashboard leaves no torrent polling at all. One narrow case is unchanged: a request already in flight when navigation starts can still apply once to the incoming page before its own poller corrects it on the next beat.
- **qBittorrent page could snap back to the previous page of torrents right after you changed page.** The 3-second poll aborts any request still in flight, but `abort()` does nothing once the response has already arrived — common on a fast local qBittorrent, where the round-trip can beat the click-to-JS-execution delay. A periodic poll issued for the *old* page could therefore land after the click's own response and repaint the list with the stale page. Each request now carries a sequence number and only the newest one is allowed to apply. Thanks to [@jeromelefeuvre](https://github.com/jeromelefeuvre) ([#17](https://github.com/ndandan/Prismarr/pull/17)).
- **Server widget parity "Last check" no longer lags the real check by days.** The Unraid API's `parityHistory` reads `/boot/config/parity-checks.log`, which Unraid itself only appends while its webGui Main page is open in a browser (the writer is an nchan daemon that exits without listeners) — so a check that finished overnight kept showing the previous entry (live-observed: "3 weeks ago" the morning after a clean 44 h check). The parity tile now falls back to the kernel's own completion stamp: when no check is running and `vars.sbSynced2` is newer than the newest history entry, the last-check line is synthesized from it (date, error count from `sbSyncErrs`, status from `sbSyncExit`). Duration is omitted in that window — `sbSynced` can mark a resume rather than the true start — and the richer history entry takes over again once Unraid backfills the log.
...
- **Dead space in the External Services settings cards.** The service-card grid on the admin settings "Services externes" tab allocated every CSS-grid row track to its tallest card, so a short card (Deluge, Houndarr) left an empty gap beside/below a tall neighbour (qBittorrent, UniFi) and an odd last card (NZBGet) left the whole second column empty. The grid is now CSS multi-column packing (`columns: 2 320px`), which flows cards into balanced stacks with no row tracks — cards pack tightly by height. Responsive behaviour is unchanged (drops to one column under ~660px); the multi-instance Radarr/Sonarr cards still span both columns via `column-span: all`.
- **Light theme presets had unreadable custom components.** Four of the 17 theme presets set `light` (catppuccin_latte, solarized_light and two others), so `data-bs-theme="light"` is reachable from the Theme picker — but several fork-built components hardcoded dark colour literals, leaving near-invisible text on a light background (calendar date numbers / day-of-week labels / filter pills, the Jellyseerr hero gradient, Prowlarr tag chips and capability labels, changelog text, the series-detail header). Those literals are now theme tokens (or pinned opaque-dark where the surface is deliberately dark, e.g. the poster-overlay hero), so every light preset renders legibly. Media-card titles were left as-is — they sit on a dark poster gradient in both themes.
- **Calendar episode colours contradicted the filter legend.** Episode event blocks used a colour family unrelated to the "Series" filter chip, so the calendar legend didn't match the events it described; episodes now render in the blue/cyan family that matches their legend chip.

### Performance
- **Opt-in FrankenPHP/Symfony worker mode (`PRISMARR_WORKER`).** Setting `PRISMARR_WORKER=1` boots the Symfony kernel once and keeps it resident, so requests skip the per-request kernel bootstrap (the single biggest remaining per-request cost). It is **OFF by default** — unset, the container runs exactly as before in classic single-request mode. `PRISMARR_WORKER_NUM` optionally pins the worker thread count (default: FrankenPHP's 2× CPU). No image rebuild is needed to toggle it. Symfony 8's default runtime natively drives FrankenPHP's worker loop, so no `APP_RUNTIME` or extra Composer package is involved. Correctness across the shared-kernel boundary is handled by every request-scoped service implementing `ResetInterface` (Symfony's `services_resetter` clears their per-request state between requests); this release closes the last two gaps — `HealthService` (in-process health memo + cache-generation token) and `DashboardController` (per-request movie/series library memo) now reset too, which also removes a latent staleness edge in classic mode.
- **OPcache preloading of the framework + app class graph.** `php.ini` now sets `opcache.preload` to Symfony's `config/preload.php` (backed by the compiled `var/cache/prod` preload file that the image's build-time `cache:warmup` already generates), so ~1,300 framework/app classes are linked into shared memory once at boot instead of on first use. FrankenPHP reports `PHP_SAPI` as `frankenphp` (not `embed`), so Symfony's preload file runs rather than short-circuiting — verified in a fresh container (1336 scripts preloaded, clean boot, health endpoint OK). Since the code is baked into the image and `opcache.validate_timestamps=0`, the preloaded graph stays valid for the container's lifetime.
...
- **One dashboard poll request instead of five.** The dashboard's live widgets (Plex activity every 10 s; service health, Unraid server and UniFi network every 30 s; Houndarr totals every 60 s) each ran their own `setInterval` and their own fragment request, so at every 30 s alignment up to four HTTP requests fired at once. A single coalescing scheduler now ticks at the greatest common divisor of the cadences and batches every widget due on that tick into one `/tableau-de-bord/widgets?w=…` request returning a `{ name: html }` map — cutting steady-state polling from ~13 to ~6 requests per minute and collapsing the aligned burst to a single request. Cadences are unchanged (declared per widget via `data-dash-poll`), initial page hydration still fetches each fragment in parallel for fast first paint, and the per-widget fail-open / hidden-tab / Turbo-singleton behaviour is preserved.
...
### Added
- **UniFi Network dashboard widget (fork-only).** Admin-only dashboard section fed by a UniFi OS console (Network 9.0+, read-only local API key): live WAN download/upload, client counts (wired/wireless/guest), a 24-hour usage graph, and infrastructure status (gateway/switches/APs online, gateway CPU/RAM). New settings card under Monitoring with kill switch, site name, and skip-TLS option; UniFi health chip (admin-gated) in the topbar/dashboard.
- **UniFi Network tab (`/unifi`, fork-only).** A full admin-only network-operations page over the classic UniFi Network API, going well beyond the dashboard widget: WAN/gateway/client tiles, 7-day traffic and 30-day speedtest trends rendered as server-side SVG (no charting library on the wire), a device inventory, an RF environment view (per-AP radio table plus neighbouring-AP scan), VLAN inventory, wireless clients, live talkers, top clients, and a DHCP-reservation mismatch list. `ROLE_ADMIN` gates the whole controller — network topology and per-client detail aren't for regular users. Three readers each own one endpoint group and its own cadence (live 10 s, infrastructure 60 s, history 300 s), so each upstream endpoint is hit once per cycle no matter how many panels consume it, and a page-scoped coalescing poller batches every region due on a tick into a single `/unifi/api/panels` request. Read-only throughout, and fail-soft by design: every panel answers `200` with an empty state rather than letting a sick console 500 the page. Built on a two-method `UnifiFetcher` seam so the already-shipped dashboard widget cannot regress. Reuses the widget's existing UniFi settings — no new configuration.
...
- **Houndarr widget (optional).** A new read-only [Houndarr](https://github.com/av1155/houndarr) integration (URL + API key in `/admin/settings`, with per-service kill switch and Test connection) adds a dashboard stat tile with the backlog-search totals — Tracked, Eligible (highlighted), Cooldown, Unreleased and 7-day Searches — hydrating asynchronously and refreshing every 60 s, plus a Houndarr chip in the unified health list (topbar + dashboard). Houndarr's API key authorizes exactly one summary endpoint (`GET /api/v1/widget`), so the client is allow-list-normalized end to end, caches results for 45 s (a rejected key is cached too, so polling can't trip Houndarr's per-IP 429 lockout), and fails open — an unreachable Houndarr renders a muted offline state and never breaks the dashboard. The section participates in dashboard layout customization (reorder/hide) like every other section; no dedicated page, since the API exposes nothing richer.
...
- **Calendar events open the quick-look in place.** Clicking a calendar entry (or an event inside a day's overflow popup) now opens the app-global quick-look modal — resolving to the library detail for known Radarr/Sonarr items, falling back to the media page otherwise — instead of navigating straight to `/medias?open=…`. Consistent with every other surface that already opens the quick-look.
- **TMDb release dates, certifications and watch providers follow the UI locale.** Release-date country, content-rating region and watch-provider region were hardcoded FR-first everywhere, so an English UI still showed French theatrical dates and certifications. A new `TmdbClient::regionPriority(locale, …)` builds a locale-led ordering (en → US/GB/CA/AU, fr → FR/BE/LU/CA, plus es/de/pt/it) with a common fallback chain and the payload's own countries appended, deduped. Wired through every pick site in `TmdbController` and `DashboardController` via the active translator locale; unit-tested. (A French instance is unchanged — FR still leads.)
- **Shared component vocabulary across download/library/discovery pages.** Three near-duplicated UI patterns were unified into reusable Twig partials with one CSS vocabulary: the Deluge/qBittorrent/Usenet view-switchers (`_view_switcher` + `.view-switcher/.view-btn`, active state now uses the theme primary), the Films/Series/Prowlarr/Usenet stat strips and grids (`_stat_tiles` + `.stat-val/.stat-lbl`), and the Films/Series/Deluge/qBittorrent filter pills (`_filter_pills` + `.filter-pill`, active pills now a `color-mix` tint of their hue). Behaviour, data-attributes and JS hooks are preserved; the most visible change is the Films/Series filter pills going from solid to tinted-active. Also retired the infinite `dl-pulse` sidebar animation in favour of a static glow plus a state-driven pulse-on-increase (reduced-motion guarded).
## [1.1.1] - 2026-06-10
